  ...
  return (
    ...
    >
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
        <div>
          <h3 id="weekly-tasks-heading" className="text-[14px] text-white font-medium mb-16">
            Weekly Tasks
          </h3>
          <div className="flex items-end gap-4 sm:gap-6 mb-4" role="list" aria-label="Task statistics">
            {data.map((item, index) => (
              <div key={index} role="listitem" className="flex flex-col">
                ...
                <p className="text-[10px] text-white mt-1 uppercase tracking-wider">{item.name}</p>
              </div>
            ))}
          </div>
          <div 
        ...
        <div>
          <div className="flex items-center justify-between mb-4">
            <h3 id="meetings-heading" className="text-[10px] text-white font-medium">
              Scheduled Meetings
            </h3>
            <label htmlFor="meeting-timeframe" className="sr-only">Filter meetings</label>
            <select
              id="meeting-timeframe"
              value={selectedTimeFrame}
              onChange={(e) => setSelectedTimeFrame(e.target.value)}
              className="text-[9px] text-white bg-[#5E5D5D] px-2 py-1 rounded-[3px] border-none outline-none cursor-pointer focus:ring-2 focus:ring-white focus:ring-offset-2 focus:ring-offset-[#021717]"
            >
