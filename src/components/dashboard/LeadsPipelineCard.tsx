  ...
  <div className="bg-[#F9FFFF] rounded-2xl p-5 border border-border shadow-md pl-20 pr-10">
    ...
    <div className="flex items-center gap-2">
      ...
      <div className="flex flex-col gap-1 items-start">
        <span className="text-[9px] text-white px-2 py-1 rounded-[3px] font-medium" style={{ background: item.color}}>
          {item.value}
        </span>
        ...
        <div className="w-[81px] h-2 bg-gray-200 rounded-full overflow-hidden h-[10px]">
          {/* Filled portion */}
          <div className="h-full rounded-full" style={{ background: item.color, width: index === 0 ? "100%" : item.value }} />
        </div>
        <span className="text-[10px] text-[#616263] mt-3">{capitalize(item.name)}</span>
      </div>
      ))}
    </div>
  </div>
